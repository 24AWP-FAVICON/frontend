...
function ChatWindow({ selectedChat, onSendMessage }) {
  ...
  // 로컬 스토리지에 채팅 내용을 저장하는 함수
  const saveChatHistoryToLocalStorage = (chatId, messages) => {
    localStorage.setItem(`chatHistory_${chatId}`, JSON.stringify(messages));
  };

  // 대화 내역 상태가 업데이트될 때마다 로컬 스토리지에 채팅 내용을 저장
  useEffect(() => {
    if (selectedChat) {
      saveChatHistoryToLocalStorage(selectedChat.id, chatHistory);
    }
  }, [chatHistory, selectedChat]);

  // 컴포넌트가 마운트될 때 로컬 스토리지에서 채팅 내용을 불러와 chatHistory 상태로 설정
  useEffect(() => {
    if (selectedChat) {
      const savedChatHistory = JSON.parse(
        localStorage.getItem(`chatHistory_${selectedChat.id}`)
      );
      if (savedChatHistory) {
  ...
  const fetchChatHistory = (chatId) => {
    ...
    const response = simulateServerRequest(chatId); // 가상의 서버 요청
    // 서버 응답에서 메시지 추출 및 설정
    if (response.success) {
